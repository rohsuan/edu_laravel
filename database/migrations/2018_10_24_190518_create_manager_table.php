...
class CreateManagerTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('manager',function(Blueprint $table){
            ...
            $table -> string('username',20) -> notNull() -> comment('用户名');//用户名，长度20的varchar，不能为nul
            $table -> string('password') -> notNull() -> comment('密码');//密码，varchar(255)，不能为null
            $table -> enum('gender',[1,2,3]) -> notNull() -> default('1')-> comment('密码');//性别，1=男，2=女，3=保密
            $table -> string('mobile',11)-> comment('手机号');//手机号，varchar(11)
            $table -> string('email',50)-> comment('邮箱');//邮箱，varchar(50)
            $table -> tinyInteger('role_id')-> comment('角色表中的主键id');//角色表中的主键id
